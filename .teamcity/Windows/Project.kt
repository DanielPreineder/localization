...
class CarbonBuildWindows(buildName: String, configType: String, preset: String) : BuildType({
    ...
    vcs {
        root(AbsoluteId("Carbon_Localization_2_LocalisationFeatureKotlin"),"+:. => %github_checkout_folder%")
    ...
    triggers {
        vcs {
            triggerRules = "+:root=${AbsoluteId("Carbon_Localization_2_LocalisationFeatureKotlin").id}:."
    ...
    features {
        pullRequests {
            vcsRootExtId = "${AbsoluteId("Carbon_Localization_2_LocalisationFeatureKotlin")}"
            provider = github {
                authType = token {
